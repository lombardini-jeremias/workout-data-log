...
export default function ExerciseItemOrDetails({
  ...
  const [editableSets, setEditableSets] = useState(
    Array(exercise.sets.length).fill(false) // To track which sets are editable
  );

  const toggleEditableSet = (index) => {
    setEditableSets((prevEditableSets) => {
      const newEditableSets = [...prevEditableSets];
      newEditableSets[index] = !newEditableSets[index];
      return newEditableSets;
    });
  };
        ...
        <View style={styles.column}>
          ...
          {exercise.sets.map((set, index) =>
            isEditable ? (
              <TouchableOpacity
                key={index}
                onPress={() => toggleEditableSet(index)}
              >
                {editableSets[index] ? (
                  <TextInput
                    style={styles.input}
                  ...
                  <Text style={styles.setNumber}>{exercise.weight || "-"}</Text>
                )}
              </TouchableOpacity>
            ) : (
              <Text style={styles.setNumber} key={index}>
            ...
            isEditable ? (
              ...
              >
                {editableSets[index] ? (
                  ...
                  <Text style={styles.setNumber}>{exercise.reps || "-"}</Text>
                )}
              </TouchableOpacity>
            ) : (
              <Text style={styles.setNumber} key={index}>
